...
namespace IOMate.Test.Application.UseCases.Events
{
    ...
    {
        private readonly Mock<IBaseRepository<User>> _repositoryMock = new();
        private readonly Mock<IMapper> _mapperMock = new();
        private readonly GetEventsHandler<User> _handler;

        public GetEventsHandlerTest()
        {
            _handler = new GetEventsHandler<User>(_repositoryMock.Object, _mapperMock.Object);
        }
        ...
        public async Task Handle_ShouldReturnEvents_WhenEventsExist()
        {
            ...
            _repositoryMock.Setup(r => r.GetEntityEventsWithOwnerAsync(userId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(events);
            _repositoryMock.Setup(r => r.GetOwnersByIdsAsync(It.IsAny<IEnumerable<Guid>>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(owners);
